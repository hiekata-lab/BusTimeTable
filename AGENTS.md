...
- `public/data/japanese-holidays.csv`: 祝日判定用の内閣府 CSV 変換データ
...
- `scraping.py` は fallback / cross-check。標準更新手段として扱わない
...
- 祝日データは内閣府 `syukujitsu.csv` を UTF-8 に変換して `public/data/japanese-holidays.csv` に保存する
- 自動運転バスが公式 PDF で運休中なら、`※` 付き自動運転便は表示対象に含めない
